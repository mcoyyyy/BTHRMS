...
class Default_Form_employee extends Zend_Form
{
	public function init()
	{
       ...
       //HRMS MODIFIED
        ...
        $complete_address = new Zend_Form_Element_Text('complete_address');
        $complete_address->setAttrib('maxLength', 100);
        $complete_address->setLabel("Complete Address");

        $emergency_contact = new Zend_Form_Element_Text('emergency_contact');
        $emergency_contact->setAttrib('maxLength', 100);
        $emergency_contact->setLabel("Emergency Contact");

        $last_attendance = new Zend_Form_Element_Text('last_attendance');
        $last_attendance->setAttrib('maxLength', 100);
        $last_attendance->setLabel("Last Attendance");

        $government_sss = new Zend_Form_Element_Text('government_sss');
        $government_sss ->setAttrib('maxLength', 100);
        $government_sss ->setLabel("SSS Number");

        $government_tin = new Zend_Form_Element_Text('government_tin');
        $government_tin ->setAttrib('maxLength', 100);
        $government_tin ->setLabel("TIN Number");

        $government_pagibig = new Zend_Form_Element_Text('government_pagibig');
        $government_pagibig ->setAttrib('maxLength', 100);
        $government_pagibig ->setLabel("Pag-Ibig Number");

        $government_philhealth = new Zend_Form_Element_Text('government_philhealth');
        $government_philhealth ->setAttrib('maxLength', 100);
        $government_philhealth ->setLabel("PhilHealth Number");

        $gender_official = new Zend_Form_Element_Text('gender_official');
        $gender_official ->setAttrib('maxLength', 100);
        $gender_official ->setLabel("Gender");

        $marital_official= new Zend_Form_Element_Text('marital_official');
        $marital_official ->setAttrib('maxLength', 100);
        $marital_official ->setLabel("Marital Status");

        $nationality_official= new Zend_Form_Element_Text('nationality_official');
        $nationality_official ->setAttrib('maxLength', 100);
        $nationality_official ->setLabel("Nationality");





/*        $medhistory_cbc = new Zend_Form_Element_Text('medhistory_cbc ');
        $medhistory_cbc ->setAttrib('maxLength', 100);
        $medhistory_cbc ->setLabel("CBC Result");*/


/*        $medhistory_cbc = new Zend_Form_Element_Select('medhistory_cbc');
        $medhistory_cbc->addMultiOption('','Select Result');
        $medhistory_cbc->setLabel("CBC Result");
        $medhistory_cbc->setRegisterInArrayValidator(false);
        $medhistory_cbc->addValidator(new Zend_Validate_Db_RecordExists(
            array('table' => 'main_medicalhistory',
                'field' => 'id',
                'exclude'=>'isactive = 1',
            )));
        $medhistory_cbc->getValidator('Db_RecordExists')->setMessage('Selected result option is deleted.');*/
...
/*                $final_emp_id->addValidator(new Zend_Validate_Db_NoRecordExists(
		...
		$this->addElements(array($id,$userid,$reportingmanager,$empstatus,$businessunit,$department,$jobtitle,
                                         ...
                                         $disp_requi,$first_name,$last_name,$employeeNumId,$final_emp_id, $complete_address, $emergency_contact, $last_attendance, $government_sss, $government_tin, $government_philhealth, $government_pagibig, $nationality_official, $gender_official, $marital_official));
